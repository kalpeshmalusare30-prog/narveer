...
export async function getFinancialSummary(): Promise<FinancialSummary> {
  return withAction({ permission: "income.view" }, async () => {
    const [pay, inc, don, exp] = await Promise.all([
    ...
    // "Other income" = general income + donations/sponsorships.
    const otherIncome = new Prisma.Decimal(inc._sum.amount ?? 0).plus(
      new Prisma.Decimal(don._sum.amount ?? 0),
    );
